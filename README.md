...
[![Size](https://img.shields.io/badge/Size-%3C30KB-orange)](packages/simplijs/dist/simplijs.js)
[![Status](https://img.shields.io/badge/Status-Stable-green)](https://github.com/Pappa1945-tech/SimpliJS)
...
*   **🚀 Ultra-Light Footprint**: Full-featured reactivity, routing, and SSG in under 30KB. Smaller than a high-res favicon.
...
| **Size (Raw)** | **< 30KB** | ~130KB | ~100KB | ~20KB (compiled) | ~22KB |
...
```html
<script type="module">
  import { createApp, component, reactive } from 'https://cdn.jsdelivr.net/gh/Pappa1945-tech/simplijs@v3.2.1/packages/simplijs/dist/simplijs.min.js';
...
1.  Download [simplijs.min.js](https://cdn.jsdelivr.net/gh/Pappa1945-tech/simplijs@v3.2.1/packages/simplijs/dist/simplijs.min.js).
...
42. **`s-query`**: Advanced cached fetching with session storage support.
    - **How to use:** `<div s-query="{ url: '/api/data', store: 'results' }">...</div>`
43. **`s-mutate`**: Declarative POST/PUT requests triggered by user interaction.
    - **How to use:** `<button s-mutate="{ url: '/api/update', body: { id: 1 } }">Update</button>`
44. **`s-loading`**: UI blocks that display only during an active network request.
    - **How to use:** `<div s-loading>Loading...</div>`
45. **`s-error` (fetch)**: UI blocks that display if a network request fails.
    - **How to use:** `<div s-error>Fetch failed!</div>`

#### **COMPONENTS & SLOTS**
46. **`s-component`**: Mounts a registered JS component onto a div or custom tag.
    - **How to use:** `<div s-component="'my-button'"></div>`
47. **`s-prop`**: Passes reactive parent data down to child components.
    - **How to use:** `<my-user s-prop:name="user.name"></my-user>`
48. **`s-slot`**: Projects content into named slots within custom components.
    - **How to use:** `<my-card><h1 s-slot="title">Hello</h1></my-card>`

#### **SPA ROUTING**
49. **`s-route`**: Defines a path-based template directly in the DOM.
    - **How to use:** `<div s-route="/home">...</div>`
50. **`s-view`**: The dynamic outlet where active routes are rendered.
    - **How to use:** `<main s-view></main>`
51. **`s-link`**: History-aware links that navigate without page reloads.
    - **How to use:** `<a s-link="/about">About</a>`

#### **PERFORMANCE & OPTIMIZATION**
52. **`s-lazy`**: Deferred loading for images/assets using Intersection Observer.
    - **How to use:** `<img s-lazy="'path/to/img.jpg'">`
53. **`s-memo`**: Skips DOM re-scans if the memo variable hasn't changed.
    - **How to use:** `<div s-memo="items.length">...</div>`
54. **`s-ref`**: Binds a native DOM element directly into your reactive state object.
    - **How to use:** `<input s-ref="myInput">`
55. **`s-once`**: Freezes a UI block after its initial render for maximum speed.
    - **How to use:** `<div s-once>{time}</div>`
56. **`s-ignore`**: Tells the engine to skip a subtree (perfect for 3rd party libs).
    - **How to use:** `<div s-ignore>...</div>`
57. **`s-init`**: Runs a JavaScript expression once when the element is first initialized. Perfect for triggering initial state changes or analytics.
    - **How to use:** `<div s-init="count = 10">...</div>`
...
> [!TIP]
> Use the **@simplijs/devtools** plugin during development to see how these plugins interact with your application's reactive state in real-time. It is automatically enabled on `localhost`, or can be manually initialized using `initDevTools()`.
