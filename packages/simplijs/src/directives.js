...
// SimpliJS HTML-First Engine Initialized
...
export class SimpliDirectives {
  ...
  reg() {
    ...
    this.dir.set('s-class', (el, val, state) => effect(() => {
      ...
      if (typeof classes === 'object' && classes !== null) {
        for (const k in classes) {
          if (k.includes(' ')) k.split(' ').forEach(c => el.classList.toggle(c, !!classes[k]));
          else el.classList.toggle(k, !!classes[k]);
        }
      } else if (typeof classes === 'string') {
        el.className = classes;
      }
    }));
    this.dir.set('s-style', (el, val, state) => {
      if (val === 'tailwind') {
        if (!window.tailwind) {
      ...
      effect(() => {
        ...
        if (typeof styles === 'object' && styles !== null) for (const k in styles) el.style[k] = styles[k];
      });
    });
    this.dir.set('s-theme', (el, val, state) => effect(() => {
      const theme = this.eval(val, state);
      if (theme === 'pico') {
    ...
    this.dir.set('s-fetch', (el, val, state) => effect(() => {
      const config = this.eval(val, state);
      const url = typeof config === 'string' ? config : config?.url;
      const store = config?.store || 'data';
      if (!url) return;
      
      state.loading = true; state.error = null;
      const doFetch = () => fetch(url, config?.options)
        .then(r => {
          if (!r.ok) throw Error(`HTTP ${r.status}`);
          return r.json();
        }).then(data => { state[store] = data; state.loading = false; })
        .catch(err => { state.error = err.message; state.loading = false; });
      
      doFetch();
      if (config?.polling) {
        const interval = setInterval(doFetch, config.polling);
        el._simpliCleanup = () => clearInterval(interval);
      }
    }));
    this.dir.set('s-query', (el, val, state) => {
       // s-query is basically a read-only s-fetch with cache support
       const config = this.eval(val, state);
       const key = `sq:${config.url}`;
       const cached = sessionStorage.getItem(key);
       if (cached) state[config.store || 'data'] = JSON.parse(cached);
       this.dir.get('s-fetch')(el, val, state);
       watch(() => state[config.store || 'data'], (val) => {
          if (val) sessionStorage.setItem(key, JSON.stringify(val));
       });
    });
    this.dir.set('s-mutate', (el, val, state) => el.addEventListener('click', async () => {
       const config = this.eval(val, state);
       state.loading = true;
       try {
         const r = await fetch(config.url, { method: 'POST', body: JSON.stringify(config.body), ...config.options });
         const data = await r.json();
         if (config.store) state[config.store] = data;
         if (config.onSuccess) this.eval(config.onSuccess, state);
       } catch (err) {
    ...
    this.dir.set('s-for', (el, val, state) => {
      ...
      effect(() => {
        ...
        list.forEach((item, index) => {
          const tempState = Object.create(state);
          if (itemExp.includes(',')) {
            const [valVar, idxVar] = itemExp.split(',').map(s => s.trim());
            tempState[valVar] = item; tempState[idxVar] = index;
          } else tempState[itemExp.trim()] = item;

          const key = keyExp ? this.eval(keyExp, tempState) : index;
          
          let itemState = statesMap.get(key);
          let node = nodesMap.get(key);

          if (!itemState) {
             itemState = reactive(Object.create(state));
          }
          // Update the reused state
          if (itemExp.includes(',')) {
            const [valVar, idxVar] = itemExp.split(',').map(s => s.trim());
          ...
          nextStatesMap.set(key, itemState);
          nextNodesMap.set(key, node);
          
          // Move node into correct position before anchor
    ...
    this.dir.set('s-loading', (el, val, state) => effect(() => {
       const isLoading = val ? this.eval(val, state) : state.loading;
       el.style.display = isLoading ? '' : 'none';
    }));
    this.dir.set('s-form', (el, val, state) => {
      el.addEventListener('submit', async (e) => {
        e.preventDefault();
        const config = this.eval(val, state) || {};
        const formData = new FormData(el);
        const data = Object.fromEntries(formData.entries());
        state.submitting = true; state.error = null;
        try {
          const r = await fetch(config.url || el.action, {
            method: config.method || el.method || 'POST',
            body: config.json !== false ? JSON.stringify(data) : formData,
            headers: config.json !== false ? { 'Content-Type': 'application/json' } : {}
          });
          const result = await r.json();
          if (config.store) state[config.store] = result;
          if (config.onSuccess) this.eval(config.onSuccess, state);
          el.reset();
        } catch (err) {
          state.error = err.message;
          if (config.onError) this.eval(config.onError, state);
        } finally { state.submitting = false; }
      });
    });
    this.dir.set('s-persist', (el, val, state) => {
      const key = val || el.id || 'simpli-persist';
    ...
    this.dir.set('s-markdown', (el, val, state) => effect(async () => {
       const content = this.eval(val, state);
       if (!content) return;
       if (!window.marked) {
    ...
    this.dir.set('s-include', (el, val, state) => effect(async () => {
    ...
    this.dir.set('s-layout', (el, val, state) => {
       const layoutName = this.eval(val, state);
       if (!layoutName) return;
       // Layout logic: find layout element and move this into its slot
       const layout = document.querySelector(`[s-layout-name="${layoutName}"]`);
       if (layout) {
          const slot = layout.querySelector('[s-slot="default"]') || layout;
          slot.appendChild(el);
       }
    });
    ...
    this.dir.set('s-init', (el, val, state) => {
