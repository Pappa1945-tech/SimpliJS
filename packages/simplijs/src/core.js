...
/**
 * Installs a SimpliJS plugin.
 * @param {Function|Object} plugin 
 * @param {...any} options 
 */
export function use(plugin, ...options) {
  if (installedPlugins.has(plugin)) return this;
  
  // We'll pass the whole Simpli object when calling from index.js
  // For now, we just handle the installation logic
  if (plugin && typeof plugin.install === 'function') {
    plugin.install(window.Simpli, ...options);
  } else if (typeof plugin === 'function') {
    plugin(window.Simpli, ...options);
  }
...
/**
 * Dynamically loads a module with optional fallback.
 */
