    ...
    if (hostComponent) {
      Array.from(node.attributes || []).forEach(attr => {
        if (attr.name === 'ref') {
          ...
          const nameParts = attr.name.replace(/^(@|on:)/, '').split('.');
          const eventType = nameParts[0];
          const modifiers = nameParts.slice(1);
          const handlerName = attr.value;
          node.removeAttribute(attr.name);
          node._simpliEvents = node._simpliEvents || {};
          if (!node._simpliEvents[eventType] && hostComponent[handlerName]) {
             const originalHandler = hostComponent[handlerName].bind(hostComponent);
             const handler = (e) => {
               if (modifiers.includes('stop')) e.stopPropagation();
               if (modifiers.includes('prevent')) e.preventDefault();
               originalHandler(e);
             };
             node.addEventListener(eventType, handler, { once: modifiers.includes('once') });
